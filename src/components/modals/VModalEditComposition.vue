...
const emits = defineEmits(['close', 'editComposition'])
const props = defineProps(['compositionID'])
const concretes = ref([])
const editedConcreteComposition = ref({})
const compositionDataReady = ref(false)
...
const validateEditCompositionInputs = () => {
  if (editedConcreteComposition.value.concrete_id === '') {
    invalidConcrete.value = true
  }
  if (editedConcreteComposition.value.name === '') {
    invalidName.value = true
  }
  if (editedConcreteComposition.value.description === '') {
    invalidDescription.value = true
  }
  if (editedConcreteComposition.value.parameter_a === '') {
    invalidParamA.value = true
  }
  if (editedConcreteComposition.value.parameter_b === '') {
    invalidParamB.value = true
  }
  if (
    invalidConcrete.value ||
    invalidName.value ||
    ...
    emits('editComposition', editedConcreteComposition.value)
    emits('close')
  }
}
...
onBeforeMount(async () => {
  fetchConcretes()
  const response = await compositionRequest.get(props.compositionID)
  editedConcreteComposition.value = response.data
  compositionDataReady.value = true
})
</script>
...
<template>
  <section
    @click="emits('close')"
    class="fixed flex top-0 left-0 z-10 justify-center items-center w-screen h-screen bg-black bg-opacity-80"
  >
    <Transition name="scale">
      <div
        v-if="compositionDataReady"
        @click.stop
      ...
      >
        ...
        <h2 class="text-xl font-bold text-center">
          {{ $t('modals.Edit-concrete-composition') }}
        </h2>
        <!-- concrete id -->
        <label
          for="concrete-id"
        ...
        <select
          ...
          :class="
            invalidConcrete
              ...
              : ''
          "
          v-model="editedConcreteComposition.concrete.id"
        ...
        <input
          ...
          v-model="editedConcreteComposition.name"
          name=""
          type="text"
          id="concrete-name"
        />
        ...
        <textarea
          ...
          v-model="editedConcreteComposition.description"
          type="text"
        ...
        <input
          ...
          "
          class="border border-black border-opacity-50 rounded mt-2 h-10 px-2 focus:outline-none focus:border-[var(--blue)] focus:border-2"
          v-model="editedConcreteComposition.parameter_a"
          type="number"
          id="param-a"
        />
        <!-- param b -->
        ...
        <input
          ...
          "
          class="border border-black border-opacity-50 rounded mt-2 h-10 px-2 focus:outline-none focus:border-[var(--blue)] focus:border-2"
          v-model="editedConcreteComposition.parameter_b"
          type="number"
          id
          param-b
        />
        <VButton
          @click="validateEditCompositionInputs()"
          class="mt-8 mb-4 w-fit self-center"
          :buttonText="$t('buttons.Edit-composition')"
        />
      </div>
    </Transition>
  </section>
